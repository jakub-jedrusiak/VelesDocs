...
First we get our drinks separated with some common separator (a space in this example), split it and save it in a `drinks` variable. Then we modify it with an f-string and a for loop. An f-string is a string preceded with `f`. In it, we can use variables' values if we enclose them in curly braces. After the f-string we put a shorthand loop syntax (it's called *list comprehension*). It will take every value inside the `drinks` one by one, put them in a variable called `drink`[^1] and do whatever we commanded before the `for` keyword. Notice everything is in **square brackets**.

[^1]: We could have called it otherwise. `[f"How do you like {i}?" for i in drinks]` would give the same result, but it makes more sense to call it `drink` rather than `i`, doesn't it?
